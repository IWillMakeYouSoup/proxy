...
function safeHeaders(headers: Record<string, any>): Record<string, any> {
  ...
  if (!process.env.LOG_AUTH || process.env.LOG_AUTH === 'false') {
    if (out['authorization']) {
      const parts = String(out['authorization']).split(' ');
      out['authorization'] =
        parts.length > 1 ? `${parts[0]} [REDACTED]` : '[REDACTED]';
    }
  }
  return out;
}
...
function hasBody(data: unknown): boolean {
...
export function logPair(
  ...
  const resHdrs = safeHeaders(upstreamRes.headers as Record<string, any>);
